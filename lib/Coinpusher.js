...
const INPUT_FEATURES = 11 * DATASET_STEP; //adjusts with TRADES_TO_ROW()
...
const STREAM_TO_DATASET = stream => {
    ...
    debug("::::::::::::::::: STREAM_TO_DATASET ::::::::::::::::", stream.length);
...
const TRADES_TO_ROW = (trades, factor) => {
    ...
    trades.forEach(trade => {
        row.push(trade.price * 100);
        row.push(trade.quantity / 100000);
        //row.push(TIMESTAMP_TO_HOUR(trade.timestamp) / 24);
        row.push(trade.maker ? 1 : 0);
        row.push(!trade.maker ? 1 : 0);
        row.push(trade.depth_diff / 10);
        row.push(trade.ask_s_tot / 100000);
        row.push(trade.bid_s_tot / 100000);
        row.push(trade.ask_m_tot / 1000000);
        row.push(trade.bid_m_tot / 1000000);
        //row.push(trade.ask_l_tot / 10000000);
        //row.push(trade.bid_l_tot / 10000000);
        row.push(Math.abs(10.0*trade.btc_min_p));
        row.push((trade.btc_min_p>0)?1:0);
    });
...
const ETLS = {
    ...
    ["nanobtc"]: {
        predict: trades => { //before prediction (is called)
            return TRADES_TO_ROW(trades, NANO_BTC_FACTOR);
        },
        main: row => { //before training (is mapped)
            // const trades = row.x.slice(0);
            row.x = TRADES_TO_ROW(row.x, NANO_BTC_FACTOR);
            fs.appendFileSync('./data1.txt', JSON.stringify(row.x)  + "\n\n\n" )
            row.y = TRADES_TO_ROW_OUTPUT(row.y, NANO_BTC_FACTOR);
            fs.appendFileSync('./data2.txt', JSON.stringify(row.y)  + "\n\n\n" )
            return row;
        },
...
class Coinpusher extends EventEmitter {
    ...
    _onTrade(data = {}){
        ...
        if(this.buffers[currency].length >= DATASET_STEP){
            if(this.nets[currency]){
                ...
                const predicted = ETLS[currency].alter(this.nets[currency].predict(ETLS[currency].predict(this.buffers[currency])), trade);

                data = Object.assign({}, data, {predicted});

                const diff = predicted[predicted.length - 1] - predicted[0];
                debug("predicted trade", diff, predicted[0], predicted[predicted.length - 1], predicted.length);

                this.checkConstantPrediction(currency, trade, predicted);
            }
        }
        else {
            debug("on trade waiting for ", this.buffers[currency].length)
        }
    ...
    startForNano(){
        return this.start("nanobtc", 3333);
    }
